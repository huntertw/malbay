...
hours_by_department %>% 
  ...
  labs(title = "Hours by task divided by number unique roles",
     y = "Task", 
...
hours_by_item_role %>% 
  ...
  geom_col(aes(x = Hours, y = Task, fill = Department.Alignment), 
           position = "dodge") +
  facet_grid(rows = vars(Current.Position), space = "free", scale = "free_y",
...
reactable(hm_dat, 
          ...
          columns = list(
            Task = colDef(minWidth = 200, filterable = TRUE, 
