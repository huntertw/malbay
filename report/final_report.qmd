...
### Hours by Task, Position Specialty, and Scalability
...
hours_by_tps %>%
  ...
  geom_col(aes(x = Hours, fill = Position.Specialty)) +
  facet_grid(rows = vars(Position.Specialty), 
...
```
### Hours by Task, Position Speciality, Current Position, and Scalability
...
hours_by_tpscps <- main_dat %>%
  ...
  arrange(Hours, Current.Position, Position.Specialty)
...
hours_by_tpscps %>% 
  ggplot() +
  geom_col(aes(x = Hours, y = forcats::fct_inorder(Task), fill = Position.Specialty), 
           position = "dodge") +
...
### Hours by Task, Operational Function, and Scalability
...
hours_by_tofs <- main_dat %>% 
  ...
  summarize(Hours = sum(Total.Hours), .groups = "drop") %>% 
  arrange(Operational.Function, Hours)

hours_by_tofs %>%
  # mutate(over_under_pct = Hours / Hours.Required-1) %>%
  # select(Task, Hours, Department.Alignment) %>%
  ggplot(aes(y = forcats::fct_inorder(Task))) +
  geom_col(aes(x = Hours, fill = Operational.Function)) +
  facet_grid(rows = vars(Operational.Function), 
             cols = vars(Scalability),
             scales = "free_y", space = "free", 
             labeller = label_wrap_gen(width = 20, multi_line = TRUE)) +
...
### Hours by Task, Operational Function, Current Position, and Scalability
...
hours_by_tpscps <- main_dat %>%
  ...
  summarize(Hours = sum(Total.Hours), .groups = "drop") %>% 
  # full_join(workitems_dat) %>%
  arrange(Hours, Current.Position, Operational.Function)
...
hours_by_tpscps %>% 
  ggplot() +
  geom_col(aes(x = Hours, y = forcats::fct_inorder(Task), fill = Operational.Function), 
           position = "dodge") +
