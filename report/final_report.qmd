...
hours_by_tps <- main_dat %>% 
  ...
  mutate(Task = reorder_within(
    Task, Hours, Position.Specialty)
...
hours_by_tpscps %>% 
  ggplot() +
  geom_col(aes(x = Hours, y = Task, fill = Position.Specialty), 
           position = "dodge") +
...
hours_by_tpscps %>% 
  ggplot() +
  geom_col(aes(x = Hours, y = Task, fill = Operational.Function), 
           position = "dodge") +
